...
| refusal texts for a stale, unknown or wrong-mode pick | `agents/selection_messages.py:13-27` | `packages/agent/src/selection-copy.ts` | `workers/edge/test/selection-validation.test.ts` | journey §3b step 5 | a refused pick carries the specific sentence Python minted and then discarded (PR #1296) | ☐ |
...
| SSRF boundary (private ranges, metadata address, own infrastructure, non-443, non-HTTPS) | `agents/byok_models.py:198` | `agent/egress/egress-policy.ts`, `packages/agent/src/host-address.ts` | `workers/edge/test/byok-egress-addresses.test.ts`, `workers/edge/test/byok-egress-policy.test.ts` | — | — | ☐ |
